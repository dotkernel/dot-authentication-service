...
Concrete authentication service implementation of `\Dot\Authentication\AuthenticationInterface` built using [zend-authentication](https://github.com/zendframework/zend-authentication) authentication and storage adapters. We advise you to have a quick look at how zend authentication works, by visiting the [documentation](https://zendframework.github.io/zend-authentication/)
...
It also wraps a zend-authentication adapter with the same name, so much of its initialization is linked to that adapter. We'll describe later how to configure such an adapter.
